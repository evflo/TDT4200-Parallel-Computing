...
void ftcs_solver_threaded ( int step );
void external_heat_threaded ( int step );

void* external_heat(void *threadArg);
void* ftcs_solver( void *threadArg);
...
// Argument to thread functions
typedef struct {
	int thread_id;
	int step;
}thread_data;

// Global temperature array size
size_t temperature_size;

// Mutex protecting temperature array
...
// Create and run threads for ftcs_solver
void ftcs_solver_threaded( int step ){
	
	// Initalize threads and thread data
	...
	{	
		...
		// Create thread with function and data
		error =  pthread_create(&threads[i],NULL,ftcs_solver,data);

		// Print to screen if error
		if( error != 0){
			printf("Cant create thread: %d for solving ftcs",i);
		}

	}

	// Join threads
	for (int i = 0; i < n_threads; ++i) (void) pthread_join(threads[i],NULL);
}
...
void* ftcs_solver( void *threadArg){
	...
	int threadId = myData.thread_id;
	int step = myData.step;

	// Compute local size
	int xSize = GRID_SIZE[0]/n_threads;

	// Find local starting point of iteration
	int xStart = xSize * threadId;

	// Create local temperature array
	float *local_temp = calloc(temperature_size,sizeof(float));
	
	// Compute ftcs for local array
	for(int x = xStart ; x < xStart + xSize; x++){
        for(int y = 0; y < GRID_SIZE[1]; y++){
            float* in = temperature[(step)%2];
            local_temp[ti(x,y)] = in[ti(x,y)] + material[mi(x,y)]*
                           (in[ti(x+1,y)] + 
                           in[ti(x-1,y)] + 
                           in[ti(x,y+1)] + 
                           in[ti(x,y-1)] -
    ...
    // Lock access to global array
    pthread_mutex_lock(&temperature_mutex);

    // Copy ftcs to global array
	for(int x = xStart ; x < xStart + xSize; x++){
        for(int y = 0; y < GRID_SIZE[1]; y++){
            float* out = temperature[(step+1)%2];
            out[ti(x,y)] = local_temp[ti(x,y)];
        }
    }

    // Unlock access to global array
    pthread_mutex_unlock(&temperature_mutex);

    // Free created argument
    free(threadArg);
...
// Create and run threads for external heat
void external_heat_threaded( int step ){
	...
	{	
		...
		// Create thread with function and data
		error =  pthread_create(&threads[i],NULL,external_heat,data);

		// Print to screen if error
		if( error != 0){
			printf("Cant create thread: %d for introducing external heat",i);
		}
	}
...
void* external_heat(void *threadArg){
	...
	// Compute local size
	int localXSize = ( GRID_SIZE[0]/2 )/n_threads;

	// Find local starting point of iteration
	int xStart = (GRID_SIZE[0]/4) + localXSize * threadId;

	// Find local end point of iteration
	int xEnd = xStart + localXSize;

	// Create local temperature array
	float* local_temp = calloc(temperature_size,sizeof(float));

	// Compute external heat locally
	for(int x= xStart; x<= xEnd; x++){
        for(int y= (GRID_SIZE[1]/2) - (GRID_SIZE[1]/16); y<= (GRID_SIZE[1]/2) + (GRID_SIZE[1]/16); y++){
			local_temp[ti(x,y)] = 100.0;
        }
    }
    ...
    // Copy external heat to global array
    for(int x= xStart; x<= xEnd; x++){
        for(int y= (GRID_SIZE[1]/2) - (GRID_SIZE[1]/16); y<= (GRID_SIZE[1]/2) + (GRID_SIZE[1]/16); y++){
			temperature[step%2][ti(x,y)] = local_temp[ti(x,y)];
        }
    }
...
int main ( int argc, char **argv ){
    ...
    temperature_size =(GRID_SIZE[0]+2*(BORDER))*(GRID_SIZE[1]+2*(BORDER));
    ...
    for( int step=0; step<NSTEPS; step += 1 ){
        if( step < CUTOFF ){
            external_heat_threaded ( step );
        }
        ftcs_solver_threaded( step );
