...
// Compute ftcs in parallel
void ftcs_solver( int step ){

	// Loop through array and compute ftcs in parallel
	#pragma omp parallel for schedule(static) num_threads(n_threads) shared(temperature)
...
// Compute external_heat in parallel
void external_heat( int step ){

	// Loop through region of array and set external heat in parallel
	# pragma omp parallel for schedule(static) num_threads(n_threads) shared(temperature)
	for(int x=(GRID_SIZE[0]/4); x<=(3*GRID_SIZE[0]/4); x++){
	    for(int y=(GRID_SIZE[1]/2)-(GRID_SIZE[1]/16); y<=(GRID_SIZE[1]/2)+(GRID_SIZE[1]/16); y++){
		    	
			temperature[step%2][ti(x,y)] = 100;
		}
	}
