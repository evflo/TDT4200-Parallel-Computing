...
void calculate_ocl(int* pixel, cl_context context,cl_command_queue q,cl_kernel kernel){

    // Allocate memory for result
    cl_mem memobj = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(int) * XSIZE * YSIZE,
                                   NULL, 0 );
	
	// Set up write buffer
    cl_int err = clEnqueueWriteBuffer(q, memobj, CL_TRUE, 0, XSIZE*YSIZE*sizeof(int), pixel, 0, NULL, NULL);
    
    if(CL_SUCCESS != err) clerror("Couldn't enqueue write buffer",err);
    
    // Set input arguments to kernel
    err = clSetKernelArg(kernel, 0, sizeof(cl_mem), (void *) &memobj );
    
    if(CL_SUCCESS != err) clerror("Couldn't set kernel arg0",err);
    
    err = clSetKernelArg(kernel, 1, sizeof(float), &xleft );
    if(CL_SUCCESS != err) clerror("Couldn't set kernel arg1",err);
    
    err = clSetKernelArg(kernel, 2, sizeof(float), &ylower );
    if(CL_SUCCESS != err) clerror("Couldn't set kernel arg2",err);
    
    err = clSetKernelArg(kernel, 3, sizeof(float), &step );
    if(CL_SUCCESS != err) clerror("Couldn't set kernel arg3",err);
    
    err = clSetKernelArg(kernel, 4, sizeof(int), &MAXITER );
    if(CL_SUCCESS != err) clerror("Couldn't set kernel arg4",err);
    
    err = clSetKernelArg(kernel, 5, sizeof(int), &XSIZE );
    if(CL_SUCCESS != err) clerror("Couldn't set kernel arg5",err);
    
    // Set size of work group and items
    size_t global_item_size[] = {XSIZE,YSIZE};
    size_t group_pattern[] = {XSIZE/8,YSIZE/8};
    size_t local_item_size[] = {global_item_size[0]/group_pattern[0],global_item_size[1]/group_pattern[1]};
    
    // Run kernel
    err  = clEnqueueNDRangeKernel(q, kernel, 2, NULL, global_item_size, local_item_size,
                                  0, NULL, NULL );
    if(CL_SUCCESS != err) clerror("Couldn't enqueue ND Range kernel",err);
    
    // Copy from GPU to CPU
    err = clEnqueueReadBuffer(q, memobj, CL_TRUE, 0, XSIZE * YSIZE * sizeof(int), pixel, 0, NULL, NULL);
    if(CL_SUCCESS != err) clerror("Couldn't enqueue read buffer",err);
